  ...
  for (const eff of effs) {
    ...
    switch (eff.action) {
      ...
      case 'rend': {
        if (!targetsOpponent) break;
        const resist = resistFraction(defender, 'rend');
        const armor = Math.max(0, Math.min(100, defender.armor)) / 100;
        const dmg = Math.round(defender.hp * (eff.multiplier ?? 0) * (1 - armor) * (1 - resist));
      ...
      case 'armor_increase': {
        addEffect(target, 'armor_increase', eff.multiplier ?? 0, eff.duration);
        events.push(`${target.id} +${((eff.multiplier ?? 0) * 100).toFixed(0)}% armor`);
        break;
      }
      case 'armor_decrease': {
        if (!targetsOpponent) break;
        const resist = resistFraction(defender, 'armor_decrease');
        const effectiveMult = (eff.multiplier ?? 0) * (1 - resist);
        if (effectiveMult > 0) {
          addEffect(defender, 'armor_decrease', effectiveMult, eff.duration);
          events.push(`${defender.id} −${(effectiveMult * 100).toFixed(0)}% armor`);
        }
        break;
      }
      ...
      case 'swap_prevent': {
        ...
        if (resist < 1) {
          addEffect(defender, 'swap_prevent', 1, eff.duration);
          events.push(`${defender.id} swap prevented`);
        }
        break;
      }
      case 'taunt': {
        if (!targetsOpponent) break;
        const resist = resistFraction(defender, 'taunt');
        if (resist < 1) {
          addEffect(defender, 'taunt', 1, eff.duration);
          events.push(`${defender.id} taunted`);
        }
        break;
      }
      case 'hot_contextual': {
        // Devour: heals attacker based on damage dealt (multiplier × damage dealt this move)
        if (targetsSelf) {
          const attackEff = effs.find(e => e.action === 'attack');
          if (attackEff) {
            const dealsDmg = calcDamage(attacker, defender, attackEff.multiplier ?? 1, bypassArmor, bypassDodge, removesShield, false);
            const healed = Math.round(dealsDmg * (eff.multiplier ?? 0));
            attacker.hp = Math.min(attacker.maxHp, attacker.hp + healed);
            events.push(`${attacker.id} devours ${healed} HP`);
          }
        }
        break;
      }
