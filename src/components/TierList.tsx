...
function shareLink(uuid: string, level: number) {
  return `/?share=${uuid}~${level}~0~0~0~0`;
}
...
function CreatureLink({ uuid, name, level }: { uuid: string; name: string; level: number }) {
  return (
    <a
      href={shareLink(uuid, level)}
        ...
        <div className="p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <a
                href={shareLink(entry.uuid, level)}
            ...
            <div className="mb-5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {entry.beats.map(uuid => (
                  <CreatureLink key={uuid} uuid={uuid} name={nameMap.get(uuid) ?? uuid} level={level} />
                ))}
              </div>
            </div>
          )}
...
export default function TierList({ entries, level, computedAt }: Props) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      {detail && (
        <DetailPanel entry={detail} nameMap={nameMap} level={level} onClose={() => setDetail(null)} />
      )}
